...
class CinemaScheduleRepository {
    val sessionsScheduleList: List<Cinema> by lazy { initCinemaList() }

    fun fetchCinemaList(): Single<List<Cinema>>{
        return Single.just(sessionsScheduleList).subscribeOn(Schedulers.io())
    }
